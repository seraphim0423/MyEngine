...
IDXGISwapChain              *g_pSwaphain = nullptr;
...
void InitGraphics() {
    VERTEX OurVertices[] = {
        {XMFLOAT3(.0f,.5f,.0f),XMFLOAT4(1.f,.0f,.0f,1.f)},
        {XMFLOAT3(.45f,-.5f,.0f),XMFLOAT4(.0f,1.f,.0f,1.f)},
        {XMFLOAT3(-.45f,-.5f,.0f),XMFLOAT4(.0f,.0f,1.f,1.f)},
    }
...
HRESULT CreateGraphicsResources(HWND hWnd) {
    ...
    if(g_pSwapchain == nullptr) {
        ...
        hr = D3D11CreateDeviceAndSwapChain(NULL,D3D_DRIVER_TYPE_HARDWARE,NULL,0,FeatureLevels,
            _countof(FeatureLevels),D3D11_SDK_VERSION,&scd,&g_pSwapchain,&g_pDev,&FeatureLevelSupported,&g_pDevcon);

        if(hr == E_INVALIDARG) {
            hr = D3D11CreateDeviceAndSwapChain(NULL,D3D_DRIVER_TYPE_HARDWARE，NULL,0,&FeatureLevelSupported,
                1,D3D11_SDK_VERSION,&scd,&g_pSwapchain,&g_pDev,NULL,&g_pDevcon);
        }
...
void RenderFrame() {
    const FLOAT clearColor[] = {.0f,.2f,.4f,1.f};
...
int WINAPI WinMain(HINSTANCE hInstance,HINSTANCE hPrevInstance,LPTSTR lpCmdLine,int nCmdShow) {
    ...
    hWnd = CreateWindowEx(0,_T("WindowClass1"),_T("Hello, Engine![Direct 3D]"),WS_OVERLAPPEDWINDOW,100,100,SCREEN_WIDTH,SCREEN_HEIGHT,
